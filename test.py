...
def update_board(board, current_player):
    ...
    while True:
        ...
        if not check_move(board, from_col, from_row, to_col, to_row):
...
def check_move(board, from_col, from_row, to_col, to_row):
    # pawn
    if board[from_row][from_col][1] == 'P':
        if abs(to_row - from_row) == 1:
            return True
        
        if from_row == 1 and board[from_row][from_col][0] == 'w' or from_row == 6 and board[from_row][from_col][0] == 'b':
